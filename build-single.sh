...
docker pull cgutman/moonlight-embedded-packaging:$TAG_NAME
PULL_EXIT_CODE=$?
...
if [ $PULL_EXIT_CODE -eq 0 ]; then
  echo Using pre-built Docker image - cgutman/moonlight-embedded-packaging:$TAG_NAME
else
  echo Pre-built image not available - building cgutman/moonlight-embedded-packaging:$TAG_NAME
...
if [ $PULL_EXIT_CODE -ne 0 ] && [ "$GITHUB_REF" == "refs/heads/master" ]; then
  docker push cgutman/moonlight-embedded-packaging:$TAG_NAME
fi